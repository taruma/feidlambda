...
[team@dev.fiako.engineering](mailto:team@dev.fiako.engineering)
...
- Umumnya nilai $\mathbf{y}$ dihitung dengan formula `=x+2` dengan asumsi $\mathbf{y}$ selalu merupakan **angka** $\{ \mathbf{x} \in \mathbb{R} \}$.
...
- Bagaimana jika $\mathbf{x}$ memiliki elemen yang bukan angka yaitu _text_ / _string_?
...
\begin{equation}
  ...
  =
  \begin{cases}
    \texttt{TRUE} & \mathbf{x} \mapsto f : \mathbf{x} \in \mathbb{R} \rightarrow \mathbb{R}\\
...
$$
\texttt{include} = \mathbf{i} \rightarrow \mathbf{i} \in \mathbb{Y}^{k}, \{k = m \lor k = n\}
$$
...
$$
$$
\mathbb{Y} = \{\textrm{seluruh nilai } y \textrm{ di mana } y \in \{ \texttt{TRUE}, \texttt{FALSE}\}\}
$$
...
= 
\begin{bmatrix}
  \textrm{A} & \textrm{AB} & \textrm{ABA}\\
  \textrm{B} & \textrm{BC} & \textrm{BCB}\\
  \textrm{C} & \textrm{CD} & \textrm{CDC}\\
  \textrm{D} & \textrm{DE} & \textrm{DED}\\
  \textrm{E} & \textrm{EF} & \textrm{EFE}\\
  \textrm{F} & \textrm{FG} & \textrm{FGF}\\
  \textrm{G} & \textrm{GI} & \textrm{GIG}\\
\end{bmatrix}
$$
