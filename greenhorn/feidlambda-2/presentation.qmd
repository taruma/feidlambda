---
title: |
  Greenhorn: feidlambda II
subtitle: |
  _**Exploring Dynamic Array + User Defined Lambda (UDL) with `LAMBDA`**_
date: |
  _`25 Oktober 2022`_<br>
  [Update: _`27 Oktober 2022`_]
author: |
...
Agenda untuk _Greenhorn feidlambda II: Exploring Dynamic Array + User Defined Lambda (UDL) with `LAMBDA`_
...
Fungsi _logical_ dan Fungsi `IS*(...)`
...
$$ \mathbf{ \mathbf{vector} \leftrightarrow \mathbf{x} \leftarrow \left[ \texttt{TRUE}, \texttt{TRUE}, \texttt{FALSE} \right]}$$
...
- `NOT(TRANSPOSE(vector))`<br> 
  $\Rightarrow$ `NOT({TRUE; TRUE; FALSE})`<br> 
  $\Rightarrow$ `{FALSE; FALSE; TRUE}`
- Jika input berupa _logical vector/array_ saja (`NOT(logical_vector)`), maka akan mengevaluasi seluruh elemen pada _logical vector/array_ (`AND(elemen_1, elemen_2, ...)`). 
...
- Jika input berupa _logical vector_ saja (`XOR(logical_vector)`), maka akan mengevaluasi seluruh elemen pada _logical vector_ (`XOR(elemen_1, elemen_2, ...)`). 
...
- Fungsi `INT(...)` merupakan _Dynamic Array Formula_, sehingga jika _input_ berupa _vector/array_ akan dievaluasi setiap elemennya.
- `INT({TRUE, FALSE, FALSE, TRUE})` $\Rightarrow$ `{1, 0, 0, 1}`. 

## _Logical_ dan operator matematik {.smaller}

- _Logical vector_ akan diubah menjadi _numeric vector_ ketika bertemu operator matematik (`+`, `*`, `-`, `/`).
- Perubahan _logical_ menjadi _number_ sama dengan proses penggunaan fungsi `INT(...)`.
- Operator matematik kali `*` dan tambah `+` dapat mengindikasikan fungsi _logical_ `AND` (`*`) dan `OR` (`+`). 
...
# Operator kali `*` ($\times$) sebagai `AND`

## Operator kali `*` sebagai `AND` (1)
...
# Operator tambah `+` ($+$) sebagai `OR`

## Operator tambah `+` sebagai `OR` (1)

- Berlaku seperti pertambahan pada umumnya.
...
- Setiap _logical array_ bertemu dengan operator matematik, hasilnya **selalu _numeric array_**.
- Jika _numeric array_ digunakan sebagai _conditional_, maka akan dikonversi menjadi _logical array_. 
